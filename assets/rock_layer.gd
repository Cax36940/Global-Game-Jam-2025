...
func update_left_rock(delta):
	...
	if number_points_to_change > 0 :
		...
		for i in range(number_points_to_change + 1, 1, -1):
			...
			points[i] = Vector2(width - (global_position.x - init_global_position.x) * velocity_factor + left_noise.get_noise_1d(begin_y + y_offset) * 20, points[number_points_to_change + 2].y - spacing * number_added_point)
	left_rock.polygon = points
...
func update_right_rock(delta):
	...
	if number_points_to_change > 0 :
		...
		for i in range(number_points_to_change + 1, 1, -1):
			y_offset -= spacing
			number_added_point += 1
			points[i] = Vector2(get_viewport_rect().size.x - width - (global_position.x - init_global_position.x) * velocity_factor + right_noise.get_noise_1d(begin_y + y_offset) * 20, points[number_points_to_change + 2].y - spacing * number_added_point)
	right_rock.polygon = points
